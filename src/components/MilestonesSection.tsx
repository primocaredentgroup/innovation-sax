...
export function MilestoneForm({
  ...
  const suggestedDate = useMemo(() => {
    const d = lastMilestone?.targetDate
      ? new Date(lastMilestone.targetDate)
      : new Date()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await onCreate({
      ...
      targetDate: new Date(dateToUse).getTime()
    })
  }
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            Suggerita: 2 settimane dopo l&apos;ultima
          </p>
        </div>
      </div>
...
function getSuggestedTargetDateStr(previousMilestone: MilestoneDoc | null): string {
  const d = previousMilestone?.targetDate
    ? new Date(previousMilestone.targetDate)
    : new Date()
  d.setDate(d.getDate() + 14)
  return d.toISOString().slice(0, 10)
}
...
export function MilestoneRow({
  ...
  previousMilestone: MilestoneDoc | null
  isEditing: boolean
  isOverdue: boolean
  isSelected?: boolean
  onEdit: () => void
  ...
  const suggestedTargetStr = getSuggestedTargetDateStr(previousMilestone)
  ...
  if (isEditing) {
    return (
      <form
        onSubmit={async (e) => {
          e.preventDefault()
          const dateToUse = editTarget || suggestedTargetStr
          await onUpdate({
  ...
  const content = (
    <div
      ...
      } ${milestone.completed ? 'ring-1 ring-green-500/20 dark:ring-green-500/30' : ''} ${isSelected ? 'ring-2 ring-blue-500 dark:ring-blue-400' : ''}`}
    >
      ...
      <div className="flex flex-wrap items-center gap-3">
        <label className="flex items-center gap-2 cursor-pointer select-none group">
          ...
          />
          <span className="text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:hover:text-gray-100 transition-colors">
            Completata
          </span>
        </label>
...
export function MilestonesSection({
  ...
  milestonesData: MilestonesData | null | undefined
  showProgress?: boolean
  initialPercentComplete?: number
  selectedMilestoneId?: Id<'coreAppMilestones'> | null
  onMilestoneSelect?: (id: Id<'coreAppMilestones'> | null) => void
}) {
  ...
  return (
    <div className="space-y-4">
      {showProgress && (
        <>
          <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase">Progresso</h3>
      ...
      <div className={showProgress ? 'pt-4 border-t border-gray-200 dark:border-gray-700' : ''}>
          ...
          <div className="space-y-3">
            {milestonesData.milestones.map((m, idx) => {
              const isOverdue =
                ...
                !m.completed
              const previousMilestone = idx > 0 ? milestonesData.milestones[idx - 1] : null
              return (
                <MilestoneRow
                  key={m._id}
                  milestone={m}
                  ...
                  isOverdue={isOverdue}
                  isSelected={selectedMilestoneId === m._id}
                  ...
                  onDelete={async () => {
                    if (confirm('Eliminare questa milestone?')) {
                      ...
                      setEditingMilestoneId(null)
                      if (selectedMilestoneId === m._id) {
                        onMilestoneSelect?.(null)
                      }
                    }
                  }}
                  onSelect={onMilestoneSelect ? () => onMilestoneSelect(m._id) : undefined}
                />
              )
            })}
          </div>
